...
class AnonsFragment : Fragment() {
    private lateinit var viewModel: AnonsViewModel
    ...
    private lateinit var adapter: AnonsAdapter
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel = ViewModelProvider(this)[AnonsViewModel::class.java]
        setupAnimeList()
        setupSwipeToRefresh()
    }
    ...
    private fun observeAnimeList(adapter: AnonsAdapter) {
    ...
    private fun observeLoadState(adapter: AnonsAdapter) {
        lifecycleScope.launch {
    ...
    private fun handleScrollingToTopWhenLoaded(adapter: AnonsAdapter) = lifecycleScope.launch {
    ...
    private fun handleListVisibility(adapter: AnonsAdapter) = lifecycleScope.launch {
        getRefreshLoadStateFlow(adapter)
            .simpleScan(count = 3)
    ...
    private fun getRefreshLoadStateFlow(adapter: AnonsAdapter): Flow<LoadState> {
        return adapter.loadStateFlow.map { it.refresh }
    }

    companion object {
        fun newInstance() = AnonsFragment()
    }
}